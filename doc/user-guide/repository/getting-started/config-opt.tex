...
There are multiple options to configure the \skbacft{vdm:skb}. The following list
...
\begin{itemize}
  ...
  \item Create a file \skbem[code]{skblocal.cfg} in your personal
        ...
        convenient way to configure the \skbacft{vdm:skb}. Note: you might need
...
for you. The macro comes with options for all possible settings of the \skbacft{vdm:skb}.
\opt{text}{\autoref{tab:skbconfig:options}}\opt{note}{This slide} describes all options and shows their default value.
Please note that the \skbacft{vdm:skb} can currently not handle inputs from directories outside the root hierarchy. However, one can
