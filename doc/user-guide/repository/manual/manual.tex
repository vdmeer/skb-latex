...
The \skbacft{vdm:skb} provides macros that simplify file handling and hide some \LaTeX~
...
\begin{skbnotelist}
  \item Installation, rebuilt and configuration: this part deals with the 
        ...
        of the \skbacft{vdm:skb} using configuration files and the macro \cmd{\skbconfig}.
  \item Files, figures and slides: the combination of \cmd{\skbheading} and
        ...
        and the macro \cmd{\skbslide} helps with \skbacft{org:iso:pdf} slides and annotations (if 
  ...
  \item Filenames, acronyms and references: here we deal with macros that provide 
        access to the path and filenames the \skbacft{vdm:skb} maintains, plus loading acronym and 
        reference databases.
  \item Other useful macros: there are some more macros that complete the \skbacft{vdm:skb}. There 
        ...
        \skbacft{org:iso:pdf} files and last not least macros that help dealing with optional and conditional text.
\end{skbnotelist}

For the impatient, we start with a few examples. The first one shows how to use the \skbacft{vdm:skb} to
produce a simple article. The second one exmplains how the documentation for the \skbacft{vdm:skb} is 
created using most of the \skbacft{vdm:skb} macros. Then we detail the usage of all the macros, following 
