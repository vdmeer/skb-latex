...
The documentation of this package is using most macros the \skbacft{vdm:skb} defines, thus 
...
As we have seen earlier, the \skbacft{vdm:skb} documentation comes in several parts. First, there 
...
or as a stand-alone article document (see user-guide.pdf). Since we use our \skbacft{vdm:skb} for
presentations, we provide an animated and a non-animated presentation about the \skbacft{vdm:skb}
...
explains the general concepts of the \skbacft{vdm:skb}. This part is generated using
many \skbacft{vdm:skb} macros including \cmd{\skbinput} and \cmd{\skbheading}.
The second part documents the implementation of the \skbacft{vdm:skb} package. This part
...
\cmd{\skbinput} and \cmd{\skbheading} and many other \skbacft{vdm:skb} macros.

Let's start with the first part, of which this example is again a part of.
The \skbacft{vdm:skb} documentation is split over two main directories: \skbem[code]{publish}
...
\begin{skbnotelist}
  ...
  \item Line 8\&9 use the \skbacft{vdm:skb} macros \cmd{\skbsubject} and \cmd{\skbkeywords} to set the subject
...
The interesting lines here are number 14 and 19. They use the \skbacft{vdm:skb} macro \cmd{\skbinput} to load 
...
All other lines use the \skbacft{vdm:skb} macro \cmd{\skbinput} to load files from the repository and associate a \LaTeX document
level to them. In the case of this document we use sections, subsections and subsubsections. Each of the files loaded here
with \cmd{\skbinput} will start by calling the \skbacft{vdm:skb} macro \cmd{\skbheading} to declare the heading text. For instance, the
...
The \skbacft{vdm:skb} now will translate use heading tex calling the classic \LaTeX macro \cmd{\subsection}. The rest is done by
\LaTeX as usual.
...
Finally, the file \skbem[code]{skb.dtx} uses the \skbacft{vdm:skb} macro \cmd{\skbinput} to load the files \skbem[code]{skb-titlepage.tex}
